...
module.exports = {
  async afterCreate(event) {
    try {
      ...
      strapi.log.error('Error sending alert create notification:', error);
    }
  },
  ...
  async beforeUpdate(event) {
    try {
      if (event.params.where?.id) {
        ...
        event.state.before = await strapi.db.query('api::alert.alert').findOne({
          where: { id: event.params.where.id },
        });
      }
    } catch (error) {
      strapi.log.error('Error loading alert before update:', error);
    }
  },
  ...
  async afterUpdate(event) {
    try {
      ...
      strapi.log.error('Error sending alert update notification:', error);
    }
  },
};
